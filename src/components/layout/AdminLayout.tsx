...
export const AdminLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] pt-24 flex flex-col md:flex-row">
      {/* Sidebar */}
      <aside className="w-full md:w-64 bg-white/5 border-r border-[#D4AF37]/20 shrink-0 p-6 flex flex-col gap-8 md:h-[calc(100vh-6rem)] md:sticky md:top-24">
        <div>
          <h2 className="text-white font-bold text-xl">{user?.firstName} {user?.lastName}</h2>
          <div className="inline-flex items-center gap-1.5 px-2 py-1 bg-[#D4AF37]/10 text-[#D4AF37] rounded-sm mt-2">
            <Shield size={12} />
